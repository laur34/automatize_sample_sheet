# Script to automate creation samplesheet.txt file for use with vsearch script for fusion primers and FORWARD reads only!
# New version - Forward only.
...
# That is, 1st col is Sample_name, 2nd is Fusion_COI_i5_TAG_Primer.
# Everything else (other columns, and rows not in project) should be deleted from it.

# Give the name of the corrected sheet (as csv) as the arg value, otherwise default name will be assumed.

import argparse, sys

parser = argparse.ArgumentParser(description='Transform 2-column sample sheet into first input for samplesheet.txt.')
...
def createInput1file(splsht2col):
    ...
    with open(splsht2col, 'r') as csvfile:
...
#                sys.exit()
            try:
                print(corename + "\t" + row[args.Sample_name_colname] + "\t" + row[args.i5_TAG_Primer_colname], file=open("input1.tsv","a"))
            except KeyError as err:
                print(err, "\n" + "Second column is not named Fusion_COI_i5_TAG_Primer. Please rename it, or specify current name as ARGV3.")
                sys.exit()
...
# Define fcn to check if forward fastq files are in directory and there is the same number of them as sample sheet says.
...
def main():
    ...
    sortAndJoin()
    cutColumns()
    CheckForFwdFastqs('samplesheet.txt')
    removeIntFiles()
    print("Finished.")
